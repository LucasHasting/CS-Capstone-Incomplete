...
void RiskReport::showRiskReport(){
	...
	if(risks.empty()) {
	  ...
	  for(unsigned i = 0 ; i < risks.size(); i++){
		...
		table->elementAt(row , 1)->addNew<WText>(currentRisk->getLongDescription());
		table->elementAt(row , 2)->addNew<WText>(currentRisk->getShortDescription());
		table->elementAt(row , 3)->addNew<WText>(to_string(currentRisk->getLikelihoodRank()));
		table->elementAt(row , 4)->addNew<WText>(to_string(currentRisk->getImpactRank()));
