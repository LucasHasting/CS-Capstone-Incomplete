...
bool DatabaseConnection::addUser(std::unique_ptr<User> user) {
  ...
  try {
    ...
    // check to see if every field is empty
...
DatabaseConnection::CreateRiskFromDB(Wt::Dbo::ptr<Risk>::mutator r) {
    ...
    // return the smart pointer
    return make_unique<Risk>(r->getID(), r->getCloseDate(), r->getOpenDate(),
    r->getnotes(), r->getStatus(); r->getOwner(), r->getLikelihoodRank(),
    r->getImpactRank(), r->getLongDescription(), r->getShortDescription());
}
...
bool editRisk( std::string ID, std::string closeDate, std::string openDate,
              std::string notes, std::string status, std::string owner, int likelihoodRank, int impactRank
              std::string longDescription, std::string shortDescription){

    try{
        Wt::Dbo::Transaction transaction(session);
       Wt::Dbo::ptr<Risk> risk = session.find<Risk>().where("ID = ?").bind(ID);

        risk.modify()-> setCloseDate(closeDate);
        risk.modify()-> setOpenDate(openDate);
        risk.modify()-> setNotes(notes);
        risk.modify()-> setStatus(status);
        risk.modify()-> setOwner(owner);
        risk.modify()-> setLikelihoodRank(likelihoodRank);
        risk.modify()-> setImpactRank(impactRank);
        risk.modify()-> setLongDescription(longDescription);
        risk.modify()-> setShortDescription(shortDescription);
